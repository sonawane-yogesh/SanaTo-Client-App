...
export class UploadDocumentsComponent implements OnInit {
  constructor(
    ...
    private msgObj: LogMessageService
  ) {}
  ...
  uploadDocuments() {
    let uploadUrl =
      this.uploadConfig.uploadApi.url +
      "?uploadDirName=" +
      this.uploadConfig.uploadDirName;
    let file: any = document.getElementById(this.fileCtrl);
    var files = file.files;
    if (files.length <= 0) {
      this.msgObj.showMessage(this.msgTd, "No file(s) selected!", "info");
      return false;
    }
    ...
    this.httpClient
      ...
      .subscribe(
        d => {
          ...
          this.emitResponse.emit(d);
          //   this.msgObj.showMessage(
          //     this.msgTd,
